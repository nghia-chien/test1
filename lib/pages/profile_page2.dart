...
class _ProfilePageState extends State<ProfilePage> {
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                  ...
                  fontSize: 16,
                  color: isActive ? const Color(0xFF5B67CA) : Colors.black54,
                ),
              ),
              if (isActive)
                Container(
  ...
  Widget _buildPostGrid(String collectionName) {
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection(collectionName).where('uid', isEqualTo: user?.uid).snapshots(),
  ...
  Widget _buildPostCard(DocumentSnapshot doc) {
    // ... giữ nguyên logic từ đoạn 1
    // bạn có thể paste lại phần _buildPostCard cũ vào đây
    return Container();
  }
}
...
class _SettingsSheet extends StatelessWidget {
  ...
  Widget _settingTile(IconData icon, String label, VoidCallback onTap) {
    return ListTile(
      leading: Icon(icon, color: Constants.darkBlueGrey.withOpacity(0.87)),
      title: Text(label),
      onTap: onTap,
    );
  }
